...
RUN VIRTUAL_ENV=/opt/runners/task-runner-python/.venv uv pip install pymongo
...
RUN sed -i 's/"N8N_RUNNERS_STDLIB_ALLOW": ""/"N8N_RUNNERS_STDLIB_ALLOW": "*"/' /etc/n8n-task-runners.json && \
    sed -i 's/"N8N_RUNNERS_EXTERNAL_ALLOW": ""/"N8N_RUNNERS_EXTERNAL_ALLOW": "pymongo"/' /etc/n8n-task-runners.json
